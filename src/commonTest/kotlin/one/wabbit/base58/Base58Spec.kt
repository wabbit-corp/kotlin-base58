...
class Base58Spec {
    ...
    val TEST_VECTORS =
        listOf(
            "00eb15231dfceb60925886b67d065299925915aeb172c06647".hexToByteArray() to
                "1NS17iag9jJgTHD1VXjvLCEnZuQ3rJDE9L",
            "73696d706c792061206c6f6e6720737472696e67".hexToByteArray() to
    ...
    @Test
    fun encodingByteArraysOfDifferentSizes() {
        ...
        for (size in inputSizes) {
            ...
            assertEquals(
                input.size,
                decoded.size,
                "Input and decoded output should have the same size for input of size $size",
            )
            assertTrue(
                input.contentEquals(decoded),
                "Decoding the encoded input should match the original input for size $size",
            )
        }
    }
    ...
    @Test
    fun encodingByteArrayWithLeadingZeros() {
        ...
        assertTrue(
            input.contentEquals(decoded),
            "Decoding the encoded input with leading zeros should match the original input",
        )
    }

    @Test
    fun encodingEmptyByteArray() {
        ...
        assertTrue(
            input.contentEquals(decoded),
            "Encoding and decoding an empty input should result in an empty output",
        )
    }
